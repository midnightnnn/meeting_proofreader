...
이 프로젝트는 속기사가 작성한 회의록(속기록)의 오타와 비문, 그리고 문맥상 어색한 표현을 **최신 AI 기술**을 활용해 자동으로 교정해주는 지능형 시스템입니다. 웹페이지에서 클릭 몇 번으로 전문가 수준의 검수 결과를 얻을 수 있도록 설계되었습니다.

---

## 📖 1. 프로젝트 소개 (Introduction)

일반적인 맞춤법 검사기는 "회의록"를 "회의 록"으로 잘못 고치거나, 회의 참석자의 이름을 엉뚱한 단어로 바꾸곤 합니다. 이는 **문맥**을 모르기 때문입니다.
...
3. 수정 전과 후를 **빨강/초록**으로 한눈에 비교해서 보여줍니다.
...
3. **검수 원칙**: 오타를 검수하고 수정하는 AI가 지켜야하는 규칙 및 페르소나를 설정합니다. 
4. **회의 메타데이터**: 더 정확한 교정을 위해 회의 주제나 참석자 이름을 입력창에 적어주세요. (AI가 이를 참고하여 고유명사 오타를 방지합니다.)
5. **검수 시작**: 버튼을 누르면 AI가 문서를 읽고 교정을 시작합니다.
6. **결과 확인**:
...
| **AI Brain** | **OpenAI GPT-4o** | 최고 수준의 한국어 문맥 이해 능력을 가진 모델 |
| **Logic** | **LangGraph** | 복잡한 AI의 생각 과정을 순서도처럼 제어하는 라이브러리 |
